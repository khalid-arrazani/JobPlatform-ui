...
import { Modal, Divider, TextField } from "@mui/material";
...
export default function AboutMeModal({ open, setOpen }) {
  ...
  useEffect(() => {
    setCompanyName(state.user?.profile?.companyName);
    setIndustry(state.user?.profile?.industry);
    setLocation(state.user?.profile?.Companylocation);
    setWebsite(state.user?.profile?.website);
    setCompanyDescription(state.user?.profile?.companyDescription);
    setPreview(state.user?.profile?.companyLogo.url);
  }, [state.user?.profile]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await updateProfileR(formData);
      setSnackBar({
        open: true,
        ...
        >
          <Box>
            ...
            <>
              ...
              <Dialog open={openi} onClose={() => setOpeni(false)}>
                <DialogContent>
                  <Box
                    sx={{
                      display: "flex",
                  ...
                  >
                    ...
                    <Button variant="contained" onClick={handleSavei}>
                      Save
                    </Button>
                  </Box>
                </DialogContent>
            ...
            >
              ...
              <Box sx={{ mb: "1rem" }}>
                ...
                <TextField
                  ...
                  multiline
                  rows={4}
                  placeholder="Tell candidates about your company, mission, culture, and what makes your workplace unique..."
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      borderRadius: "0.3rem",
                    },
