...
export default function ProfileDetails() {
  ...
  const fetchUser = async () => {
    ...
    try {
      let info;
      Role == "jobSeeker"
        ? (info = await BringProfileByIdJs(ProfileId))
        : Role == "recruiter"
          ? (info = await BringProfileByIdR(ProfileId))
          : null;
      setData(info);
    } catch (error) {
  ...
  return (
    <>
      {loading ? (
        ...
        <MyProfilePagejs data={data} />
      ) : Role == "recruiter" ? (
        <RecruiterProfilePage data={data} />
      ) : null}
    </>
  );
}
